...
def write_data(df, table_name):
    ...
    try:
        logger.info(f"Writing data to Delta Lake for {table_name}")
        df.write.format("delta") \
            .option("path", f"{os.environ['DELTA_LAKE_PATH']}/{table_name}") \
            .option("mergeSchema", "true") \
            .partitionBy("SALE_DATE") \
            .saveAsTable(f"{table_name}")
        logger.info(f"Wrote data to Delta Lake for {table_name}")
    except Exception as e:
        logger.error(f"Error writing data to Delta Lake for {table_name}: {str(e)}")
        raise
...
def data_quality_checks(df, table_name):
    ...
    try:
        ...
        for column, null_count in null_counts.asDict().items():
            logger.info(f"{table_name}: {column} has {null_count} null values")
